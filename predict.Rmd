...
We will start with a simple tree model. In order to estimate the out of sample error rate, we will be using k-fold cross-validation as the resampling method.  A set of models will be evaluated across a number of tuning parameters sets and accurary will be used as the metric to choose the best model.
...
tree_model <- train(
                    ...
                    , method = 'rpart'
                    , trControl = control
                    , tuneLength = 5 # Number of models to be evaluated
                    , metric = 'Accuracy'
                    )
...
From the results above, we estimate that the out of sample error rate of the model when applied to the test data is: `r sprintf('%.2f%%', (1 - acc) * 100)`, which is the average percentage of observation that were inccorrectly classified in the cross validation process.

### Plot information about the tree model

```{r}
# Plot the tree
fancyRpartPlot(tree_model$finalModel, main = '', sub = '')

# Build a data frame with variables importance in the tree model
...
Aiming to build a better model, we will now use Random Forest.
...
From the results above, we estimate that the out of sample error rate of the model when applied to the test data is: `r sprintf('%.2f%%', (1 - acc) * 100)`, which is the average percentage of observation that were incorrectly classified in the cross validation process.
