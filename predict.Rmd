...
Train a random forests model, using the train data slice and the parameters defined above. The *tuneLength* parameter is used to specify the number of models to evaluate. The best one will be chosen by the function as the final one.
...
Now that we have our model, we will validate it on the validation data slice and estimate our out-of-sample error rate.
